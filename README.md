...
## Q. wait() 없이 락을 계속 잡고 있으면 어떤 문제가 생기나요?

좌석이 이미 예약되었는데도 `wait()`를 호출하지 않고, 락을 잡은 채로 3초를 버틴다고 가정하겠습니다(예: `sleep(3000)`).

- 그 3초 동안 다른 스레드는 `cancelReservation()`에 들어올 수 없습니다.  
- 즉, **취소가 발생할 수 있는 통로가 막힙니다.**  
- 결과적으로 “취소표를 기다린다”는 행위 자체가 의미가 없어집니다.

따라서 “기다림을 하되, 다른 스레드가 취소/변경할 수 있게” 하려면  
`sleep`이 아니라 **`wait`** 가 필요합니다.
